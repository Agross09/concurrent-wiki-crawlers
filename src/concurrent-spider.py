...
#=========================================================================
def create_job_queue(starting_url, num_worker_threads):
    """
    ...
    for link in soup.find_all('a'):
        ...
        if href and len(href) > 6 and href[:6] == '/wiki/':
            ...
            if num_crawled_sites < CRAWL_SITE_LIMIT:
...
def main(starting_url):
    ...
    producer_thread = Thread (
        ...
        args=[starting_url, num_worker_threads]
    )
    producer_thread.start()

    worker_threads = []
    for _ in range(num_worker_threads):
        worker_threads.append(Thread(target=consume))
